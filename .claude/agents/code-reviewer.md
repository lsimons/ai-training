...
rewrite files. It rejects a redirect to any file but `/dev/null`. It
checks the command inside each `$(...)`, backtick pair and `<(...)` the
same way, and it rejects an unquoted `(`, `)` or brace expansion
(`{a,b}`), so quote those when they are text. The platform may not give
you the Grep and Glob tools next to Bash, so search with `grep -rn` and
list with `ls`. `AGENTS.md` is already loaded, so don't read it again.
