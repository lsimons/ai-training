...
rule ends each run. The last one is caught by nothing in the loop, and we
...
<Predict id="predict-never-answers" objective="building-agents/builds-agent-loop/implements-the-loop"
  ...
  title="Predict which rule fires" hint="Which of the checks can see a difference between this round and the last one, and which counter is the only one that moves?"
...
Repeating the same action after the same result is the loop failure
mode that shows up first in planning agents (@AEC-06), and the defense is
to remember what was tried. `rounds` gives the request and result of every
...
one round back. The step limit catches this run, at the price of two
wasted rounds, and with `max_steps` at forty the price is thirty-eight.
The exercise at the end of the lesson adds the check that catches it.
...
3. A loop runs in circles by repeating a round or by alternating between
   two, and a check that remembers earlier rounds ends both (@AEC-06).
