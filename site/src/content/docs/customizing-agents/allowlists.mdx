...
the vendor's documentation on the date shown above the lesson
...
never wins against a broader deny rule, whichever came first in the file
...
repository, so no rule you saved in any other project applies here.
...
In a fresh copy the list is empty. Close it and send this brief.
...
and choose the plain "Yes" each time. The other option, "Yes, and don't
ask again", writes a rule for you, and this lesson writes the rules by
hand first so you can see what one looks like. Expect an edit to
...
The `deny` entry does a job the brief only asked for. "Do not edit
...
shapes this lesson uses, an exact rule and a rule with one trailing `*`.
...
written with a push in mind, and it approves one anyway. Claude Code
matches everything before the first `*` as written. The words before it
are the whole limit of the rule (@Claude Code permissions).
...
reason. Claude Code
recognizes a built-in set of read-only commands, among them `ls`, `cat`,
`grep` and the read-only forms of `git`, and runs them without a prompt
in every mode (@Claude Code permissions). `git status` was never in the
...
files wins over an allow rule in any other, because Claude Code
...
approved it, because everything after `git` is the wildcard. The rule:
put the `*` after the subcommand, as in `Bash(git commit *)`, and write
one rule per subcommand the task needs. Claude Code matches the text
before the first `*` exactly, and nothing after it limits the rule
(@Claude Code permissions).
</Pitfall>
...
4. Write the rule from the tally: sort the prompts into reads, writes and
   ...
   working directory doesn't prompt and doesn't need one.
