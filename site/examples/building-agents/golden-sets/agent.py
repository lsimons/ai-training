"""The agent under test in the lesson "A golden set is the agent's regression suite".

A copy of the basic retrieval agent from the lesson "Retrieval as a tool the
agent calls": one keyword search over the twelve handbook texts in `docs/`,
then a fake model that answers from the passage it was given. The fake model
reads the prompt it gets, so a change to PROMPT_LINES changes what it does,
the way a change to a real prompt would. Every run prints the same thing and
needs no API key.
"""
...
# The prompt frame around the retrieved passage, one instruction per line.
# The lesson removes the last line and runs the golden set again.
PROMPT_LINES = [
