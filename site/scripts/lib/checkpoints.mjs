 ...
 * - a `choice`, `scenario` or `multi-choice` item is guessable from a surface
 *   cue (guessability.mjs lists the heuristics), or carries a `guessable`
 *   exemption that no heuristic needs.
 *
 ...
 * exemptions: string[] }`, where `exemptions` is one line per `guessable`
 * item with its reason, for the caller to print.
 */
export function checkCheckpoints(file, contentDir, dataDir) {
	const errors = [];
	const fail = (msg) => errors.push(msg);
	if (!existsSync(file)) return { errors: [`${file} does not exist; run site-build first`], items: 0, exemptions: [] };
	let data;
	try {
		data = JSON.parse(readFileSync(file, 'utf8'));
	} catch (e) {
		return { errors: [`${file}: not JSON: ${e.message}`], items: 0, exemptions: [] };
	}
	if (data?.version !== 1) fail(`version is ${JSON.stringify(data?.version)}, expected 1`);
	if (!Array.isArray(data?.items)) return { errors: [...errors, 'no items list'], items: 0, exemptions: [] };
	...
	return { errors, items: data.items.length, exemptions: guess.exemptions };
}
