...
| Python    | `mise run py-test`     | pytest, `tests/test_*.py`, with the coverage floor over `scripts/`   | Wrong logic in the repo's Python helpers (`scripts/`): the Vale eval, metrics, config checks, line-break rewrite, run names and wave status                        |
