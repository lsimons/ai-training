...
`site/src/pages/data/checkpoints.json.ts` from the same source scan the
review page uses. Each item has `id`, `lesson`, `kind`, `objective`,
`concepts`, `context`, `stem`, `options`, `answer`, `hint`, `reviewable`
and `revision`. S03 "Checkpoint export" gives the form of `options` and
`answer` per kind. `mise run checkpoints` (part of `mise run ci`, after
the build) reads the file back and fails when a page checkpoint is missing
from it, an item has no page, a field is missing, or a concept id isn't in
the topic YAML. The browser doesn't read the file yet. An author does
