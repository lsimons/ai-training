...
| `extends-to`      | Where a confident learner goes next: the next lesson, a specialization topic, a short, or an `https://` URL under the `url` of a bibliography entry (a Claude Academy course, say). An external entry renders as a plain link marked "(external link)"               |
