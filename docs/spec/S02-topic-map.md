...
| `Claude Code memory`           | Anthropic's Claude Code documentation page "How Claude remembers your project" at `code.claude.com/docs/en/memory`                                                  | Proprietary                 | Loading, folders, notes, imports, enforcement, pruning, re-checked by `review-by`; no text adapted    |
...
| `Claude Code best practices`   | Anthropic's Claude Code documentation page "Best practices for Claude Code" at `code.claude.com/docs/en/best-practices`                                             | Proprietary                 | Plan-first, failing-test, resume and restart advice, re-checked by `review-by`; no text adapted       |
...
| `Bandit PyPI`                  | The PyPI page for Bandit 1.9.4, which states the Python versions that release needs                                                                                 | Apache-2.0                  | Version requirement as a fact, re-checked by `review-by`; no text adapted                             |
...
| `Anthropic effective agents`   | Anthropic's engineering article "Building effective agents" at `anthropic.com/engineering/building-effective-agents`, on workflows versus agents                    | Proprietary                 | The workflows-versus-agents advice paraphrased in our own words; no text adapted                      |
...
| `Codex approvals`              | OpenAI's Codex documentation page "Agent approvals & security" at `learn.chatgpt.com/docs/agent-approvals-security`                                                 | Proprietary                 | Sandbox modes, approval policy and network defaults as facts; no text adapted                         |
...
| `coding-with-agents/team`          | Agents in a team             | parallel sessions, worktrees, CI integration, team hooks, review norms, attribution                                                                                                               | quality                                   | `DLAI-5`; this repo's own practice; `Academy claude-code-in-action`, `Academy ai-native-sdlc-playbook`                               |
...
| `building-agents/patterns`         | Design patterns               | ReAct, reflection, planning, plan-then-execute vs reactive, hierarchical planning                                                      | agent-loop                                            | `AEC-04`, `AEC-06`; `DLAI-11` M2, M5; `Academy building-with-the-claude-api`; `Anthropic effective agents`                        |
