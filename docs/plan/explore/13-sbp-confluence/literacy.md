...
| -------------- | ---------------------------------------------------------------------- | ------------------------------------- |
| **Understand** | Read, watch or take a course; write a definition; draw a diagram.      | A reflection paragraph, a certificate |
...
and its checkpoints; **Do** and **Judge** are the two flavours of
[Exercise](../../../spec/S03-lesson-authoring.md). A course whose exercises
are all "do" has no place where the learner forms an opinion; a course whose
...
The three-way split is also a decent lens on a whole curriculum. Listing
...
## 2. A shape for exercise text

Nearly every task written in the programme's second half used one template,
and it is worth adopting as the default for exercise prose:
...
- **Make the proof the artifact.** A screenshot or a short text is the whole
  ...
  got a handful of completions at most.
...
| Any single "advanced" prompting technique                   | 2 to 10     |
...
- **Tasks that use a public interactive challenge work.** Three external
  sites were used as weekly tasks and are worth listing as `Source` entries
...
- **Rewards shaped behavior more than learning did.** Points scaled with
  business impact, so a sales pitch or a process automation was worth
  fifty to a hundred times a learning task, and the biggest single spike in
...
## 4. Exercise ideas worth writing
...
1. Name the two exercise flavours "do" and "judge" in S03, or keep a single
...
2. The completion figures are from one organization and one year. Quote them
   ...
   background only?
3. The three public challenge sites are third-party and could change or
